...
cuda_iter_tier_rings_new (int num_procs)
{
  ...
  cuda_iter_tier_ring_t *ring;
  cuda_iter_tier_t  *tier;

  tier_rings = sc_array_new (sizeof (cuda_iter_tier_ring_t));
  ...
  for (i = 0; i < tier_level_max; i++) {
    ring = (cuda_iter_tier_ring_t *) sc_array_index_int (tier_rings, i);
    ring->next = 0;
    sc_array_init (&(ring->tiers), sizeof (cuda_iter_tier_t));
    sc_array_resize (&(ring->tiers), (size_t) tier_ring_max);
    for (j = 0; j < tier_ring_max; j++) {
      tier = (cuda_iter_tier_t *) sc_array_index_int (&(ring->tiers), j);
      tier->key = NULL;
    }
  }
...
cuda_iter_tier_rings_destroy (sc_array_t * tier_rings)
{
  ...
  for (zz = 0; zz < tier_rings->elem_count; zz++) {
    ring = (cuda_iter_tier_ring_t *) sc_array_index (tier_rings, zz);
                        ...
                        p4est_quadrant_t * q)
{
  ...
  if (level >= (int) tier_rings->elem_count) {
    cuda_iter_tier_update (view, level, next_tier, shift);
    return;
  }
  ring = (cuda_iter_tier_ring_t *) sc_array_index_int (tier_rings, level);

  limit = (int) ring->tiers.elem_count;
  for (i = 0; i < limit; i++) {
    tier = (cuda_iter_tier_t *) sc_array_index_int (&(ring->tiers), i);
    key = tier->key;
    if (key == NULL) {
      P4EST_ASSERT (ring->next == i);
      cuda_iter_tier_update (view, level, next_tier, shift);
      memcpy (tier->array, next_tier, CUDA_ITER_STRIDE * sizeof (size_t));
      tier->key = q;
      ring->next++;
      ring->next %= limit;
      return;
    ...
    if (q == key) {
      memcpy (next_tier, tier->array, CUDA_ITER_STRIDE * sizeof (size_t));
      return;
    }
  }
  ...
  tier = (cuda_iter_tier_t *) sc_array_index_int (&(ring->tiers),
                                                   ring->next++);
  memcpy (tier->array, next_tier, CUDA_ITER_STRIDE * sizeof (size_t));
  tier->key = q;
  ring->next %= limit;
                          ...
                          p4est_ghost_t * ghost_layer, int num_procs)
{
  ...
  for (i = 0; i < alloc_size; i++) {
    loop_args->index[i] =
      P4EST_ALLOC (size_t, (P4EST_QMAXLEVEL + 1) * CUDA_ITER_STRIDE);
    if (i & 1) {
  ...
  loop_args->tier_rings = cuda_iter_tier_rings_new (num_procs);
...
cuda_iter_loop_args_destroy (p4est_iter_loop_args_t * loop_args)
{
  ...
  cuda_iter_tier_rings_destroy (loop_args->tier_rings);
  P4EST_FREE (loop_args);
}

/* initialize loop_args for a volume search */
static void
cuda_iter_init_loop_volume (p4est_iter_loop_args_t * loop_args,
...
static void
cuda_iter_init_loop_face (p4est_iter_loop_args_t * loop_args,
...
static void
cuda_iter_init_loop_outside_face (p4est_iter_loop_args_t * loop_args,
                                   p4est_topidx_t t, p4est_t * p4est,
                                   p4est_ghost_t * ghost_layer)
{
  const int           local = 0;
...
#ifdef P4_TO_P8
static void
p8est_iter_init_loop_edge (cuda_iter_loop_args_t * loop_args,
...
static void
cuda_iter_init_loop_corner (p4est_iter_loop_args_t * loop_args,
                             p4est_t * p4est, p4est_ghost_t * ghost_layer,
                             p4est_iter_corner_info_t * info)
{
  ...
  for (zz = 0; zz < limit; zz++) {
    side = cuda_iter_cside_array_index (&info->sides, zz);
    t = side->treeid;
    tree = p4est_tree_array_index (trees, t);
    local_quads = &(tree->quadrants);
    loop_args->index[zz * 2 + local][0] = 0;
                         ...
                         const int *start_idx2, int old_num, int factor)
{
  ...
  int                 i, j, idx2 = loop_args->level * CUDA_ITER_STRIDE;
...
static void
cuda_iter_init_corner (p4est_iter_corner_args_t * args,
                        p4est_t * p4est, p4est_ghost_t * ghost_layer,
                        ...
                        int c)
{
...
#endif
      for (j = 0; j < count; j++) {
        cside = cuda_iter_cside_array_index_int (&(info->sides), j);
...
#endif
    for (i = 0; i < count; i++) {
      cside = cuda_iter_cside_array_index_int (&(info->sides), i);
      nt = cside->treeid;
      nc = cside->corner;
      for (j = 0; j < P4EST_DIM; j++) {
        if (cside->faces[j] == -1) {
          ...
          for (k = 0; k < count; k++) {
            ...
            cside2 = cuda_iter_cside_array_index_int (&(info->sides), k);
  ...
  cuda_iter_init_loop_corner (loop_args, p4est, ghost_layer, info);
}

static void
cuda_iter_reset_corner (p4est_iter_corner_args_t * args)
{
                      ...
                      p4est_iter_corner_t iter_corner)
{
   ...
   * this level */
  level_idx2 = level * CUDA_ITER_STRIDE;
  ...
  for (side = 0; side < num_sides; side++) {
    ...
    cside = cuda_iter_cside_array_index_int (&info->sides, side);
    cside->quad = NULL;
    cside->is_ghost = 1;
    cside->quadid = -1;
    ...
    for (side = 0; side < num_sides; side++) {
      ...
      for (dir = 0; dir < P4EST_DIM; dir++) {
        ...
        for (side2 = 0; side2 < num_sides; side2++) {
          ...
          cside2 = cuda_iter_cside_array_index_int (&info->sides, side2);
...
static void
p8est_iter_init_corner_from_edge (cuda_iter_corner_args_t * args,
...
p8est_iter_init_edge (p8est_iter_edge_args_t * args, p8est_t * p8est,
                      p4est_ghost_t * ghost_layer,
                      cuda_iter_loop_args_t * loop_args, p4est_topidx_t t,
                      int e)
{
  p4est_topidx_t      ti;
  int                 i, j, k;
...
p8est_iter_reset_edge (p8est_iter_edge_args_t * args)
{
  if (args->loop_args->loop_corner) {
    cuda_iter_reset_corner (&args->corner_args);
  }
                    ...
                    p8est_iter_corner_t iter_corner)
{
  ...
  cuda_iter_loop_args_t *loop_args = args->loop_args;
  int                 num_sides = args->num_sides;
  ...
  cuda_iter_corner_args_t *corner_args = &(args->corner_args);
      ...
      if (refine[side]) {
        ...
        if (stop_refine) {
          ...
          for (i = 0; i < 2; i++) {
             ...
             * quadrant */
            quad_idx2[side] = level_idx2 + CUDA_ITER_STRIDE + *temp_int;
            for (type = local; type <= ghost; type++) {
              st = side * 2 + type;
              first_index[st] = zindex[st][quad_idx2[side]];
              count[st] =
 ...
 */
static void
cuda_iter_init_corner_from_face (p4est_iter_corner_args_t * args,
                                  p4est_iter_face_args_t * face_args)
{
  ...
  for (j = 0; j < ntc_str; j++) {
    for (k = 0; k < limit; k++) {
      fside = cuda_iter_fside_array_index_int (&face_args->info.sides, k);
      cside = cuda_iter_cside_array_index_int (&info->sides, count);
...
p8est_iter_init_edge_from_face (p8est_iter_edge_args_t * args,
                                cuda_iter_face_args_t * face_args,
                                int dir, int side)
{
  const int           ntc_str = P4EST_CHILDREN / 2;
  int                 j, k;
...
{
  p4est_locidx_t      diff;
  const cuda_iter_face_side_t *A = (const cuda_iter_face_side_t *) a;
  const cuda_iter_face_side_t *B = (const cuda_iter_face_side_t *) b;
                      ...
                      int f)
{
  ...
  if (nt < t || (nt == t && nf < f)) {
    cuda_iter_init_face (args, p4est, ghost_layer, loop_args, nt, nf);
    return;
  }
  ...
  sc_array_init (&(info->sides), sizeof (cuda_iter_face_side_t));
...
#endif
  if (loop_args->loop_corner) {
    cuda_iter_init_corner_from_face (&(args->corner_args), args);
  }

  if (nt != -1) {
    cuda_iter_init_loop_face (loop_args, t, nt, p4est, ghost_layer);
  }
  else {
    cuda_iter_init_loop_outside_face (loop_args, t, p4est, ghost_layer);
  }
}
 ...
 */
static void
cuda_iter_init_face_from_volume (p4est_iter_face_args_t * args,
                                  ...
                                  int dir, int pos)
{
  ...
  fside = cuda_iter_fside_array_index_int (&info->sides, 0);
  fside->treeid = volume_args->info.treeid;
  fside->face = (int8_t) (2 * dir + 1);
  fside = cuda_iter_fside_array_index_int (&info->sides, 1);
  fside->treeid = volume_args->info.treeid;
...
p8est_iter_init_edge_from_volume (p8est_iter_edge_args_t * args,
                                  cuda_iter_volume_args_t * volume_args,
                                  int dir, int side)
{
  int                 i;
  int                *cc;
 ...
 */
static void
cuda_iter_init_corner_from_volume (p4est_iter_corner_args_t * args,
                                    p4est_iter_volume_args_t * volume_args)
{
  int                 i;
  p4est_iter_corner_info_t *info = &(args->info);
                        ...
                        p4est_iter_loop_args_t * loop_args, p4est_topidx_t t)
{
  ...
  for (i = 0; i < P4EST_DIM; i++) {
    for (j = 0; j < P4EST_CHILDREN / 2; j++) {
      cuda_iter_init_face_from_volume (&(args->face_args[i][j]), args, i, j);
    }
#ifdef P4_TO_P8
    if (loop_args->loop_edge) {
      for (j = 0; j < 2; j++) {
  ...
  if (loop_args->loop_corner) {
    cuda_iter_init_corner_from_volume (&(args->corner_args), args);
  }
...
cuda_iter_reset_volume (p4est_iter_volume_args_t * args)
{
  ...
  for (i = 0; i < P4EST_DIM; i++) {
    for (j = 0; j < P4EST_CHILDREN / 2; j++) {
      cuda_iter_reset_face (&(args->face_args[i][j]));
    }
#ifdef P4_TO_P8
    if (args->loop_args->loop_edge) {
      for (j = 0; j < 2; j++) {
 ...
 * a simple loop is performed */
 static void
 cuda_volume_iterate_simple (cuda4est_t * cuda4est, p4est_ghost_t * ghost_layer,
 ...
 {
   simple_new_face_cuda_iterate(cuda4est, ghost_layer, user_data_volume_cuda_api, new_iter_face_api, new_iter_quad_api);
 }
...
void
cuda_iterate_ext (cuda4est_t * cuda4est, p4est_ghost_t * Ghost_layer,
                  void* user_data,
                  user_data_for_cuda_t* user_data_volume_cuda_api,
                  p4est_iter_volume_t iter_volume,
                   ...
                   p4est_iter_corner_t iter_corner, int remote)
{
  ...
  if(cuda_iter_new_face_api != NULL) {
    if(cuda_iter_face_api != NULL) {
      ...
      cuda_new_face_iterate_simple(cuda4est, ghost_layer, user_data_volume_cuda_api, cuda_iter_new_face_api, cuda_new_iter_quad_api);
    }
  } else {
    if(cuda_iter_face_api != NULL) {
      if(cuda_iter_volume_api != NULL) {
...
//                          iter_corner);
    ...
    if (loop_args->loop_corner) {
      for (c = 0; c < P4EST_CHILDREN; c++, mask <<= 1) {
        ...
        cuda_iter_init_corner (&corner_args, p4est, ghost_layer, loop_args,
                                t, c);
        p4est_corner_iterate (&corner_args, user_data, iter_corner);
...
void
cuda_iterate (cuda4est_t * cuda4est, p4est_ghost_t * Ghost_layer,
              void* user_data,
              user_data_for_cuda_t *user_data_volume_cuda_api,
              p4est_iter_volume_t iter_volume,
              cuda_iter_volume_api_t* cuda_iter_volume_api,
              ...
              cuda_iter_face_api_t* cuda_iter_face_api,
               ...
               p4est_iter_corner_t iter_corner)
{
  cuda_iterate_ext (cuda4est, Ghost_layer, user_data, user_data_volume_cuda_api, iter_volume, cuda_iter_volume_api, iter_face, cuda_iter_face_api, NULL, NULL,
#ifdef P4_TO_P8
                     iter_edge,
#endif
                     iter_corner, 0);
               ...
               p4est_iter_corner_t iter_corner)
{
  cuda_iterate_ext (cuda4est, Ghost_layer, user_data, user_data_volume_cuda_api, iter_volume, cuda_iter_volume_api, iter_face, cuda_iter_face_api,
   cuda_iter_new_face_api,
   cuda_new_iter_quad_api,
#ifdef P4_TO_P8
                     iter_edge,
#endif
                     iter_corner, 0);
