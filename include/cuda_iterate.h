...
#ifndef CUDA_ITERATE_H
#define CUDA_ITERATE_H
...
typedef struct cuda_user_data_info
{
  void * user_data;
  size_t user_data_size;
}
cuda_user_data_info_t;
 ...
 */
typedef struct cuda_iter_volume_info
{
  p4est_t            *p4est;
 ...
 */
typedef void        (*cuda_iter_volume_t) (
  p4est_t            *p4est,
  p4est_ghost_t      *ghost_layer,
...
typedef void (*cuda_iter_volume_setup_kernel_t)(cuda_iter_volume_t *callback);

typedef struct cuda_iter_volume_api
{
  cuda_iter_volume_t callback;
  cuda_iter_volume_setup_kernel_t setup_kernel;
} cuda_iter_volume_api_t;
 ...
 * and quadid = -1.
 */
typedef struct cuda_iter_face_side
{
  ...
  int8_t              is_hanging;      /**< boolean: one full quad (0) or
                                            two smaller quads (1) */
  union cuda_iter_face_side_data
  {
    struct
    {
 ...
 */
typedef struct cuda_iter_face_info
{
  p4est_t            *p4est;
  p4est_ghost_t      *ghost_layer;
...
typedef struct cuda_new_iter_face_api
{
  cuda_new_iter_face_t callback;
 ...
 */
typedef struct cuda_iter_corner_side
{
 ...
 */
typedef struct cuda_iter_corner_info
{
  p4est_t            *p4est;
  p4est_ghost_t      *ghost_layer;
  int8_t              tree_boundary; /**< boolean: interior face (0),
 ...
 * p4est_balance()).
 */
typedef void        (*cuda_iter_corner_t) (cuda_iter_corner_info_t * info,
                                            void *user_data);
...
cuda_iter_cside_array_index_int (sc_array_t * array, int it)
{
  P4EST_ASSERT (array->elem_size == sizeof (cuda_iter_corner_side_t));
  P4EST_ASSERT (it >= 0 && (size_t) it < array->elem_count);

  return (p4est_iter_corner_side_t *)
    (array->array + sizeof (cuda_iter_corner_side_t) * (size_t) it);
}
...
/*@unused@*/
static inline p4est_iter_corner_side_t *
cuda_iter_cside_array_index (sc_array_t * array, size_t it)
{
  P4EST_ASSERT (array->elem_size == sizeof (cuda_iter_corner_side_t));
  P4EST_ASSERT (it < array->elem_count);

  return (p4est_iter_corner_side_t *)
    (array->array + sizeof (cuda_iter_corner_side_t) * it);
}
...
cuda_iter_fside_array_index_int (sc_array_t * array, int it)
{
  P4EST_ASSERT (array->elem_size == sizeof (cuda_iter_face_side_t));
  P4EST_ASSERT (it >= 0 && (size_t) it < array->elem_count);

  return (p4est_iter_face_side_t *)
    (array->array + sizeof (cuda_iter_face_side_t) * (size_t) it);
}
...
/*@unused@*/
static inline p4est_iter_face_side_t *
cuda_iter_fside_array_index (sc_array_t * array, size_t it)
{
  P4EST_ASSERT (array->elem_size == sizeof (cuda_iter_face_side_t));
  P4EST_ASSERT (it < array->elem_count);

  return (p4est_iter_face_side_t *)
    (array->array + sizeof (cuda_iter_face_side_t) * it);
}
